...
public:
    ...
    void Draw(prac::QPainter& painter) const {
        ...
        for(int i = 0; i < 6; i++){
            ...
            if (params_.depth > 0) {
                SnowFlake next_flake = GetNextLevelFlake(end, params_.rotation + 60 * i);
    ...
    SnowFlake GetNextLevelFlake(Point2D position, double angle) const {
        SnowFlakeParams next_params = params_;
        next_params.center = position;
        next_params.size = params_.size * params_.scale_factor;
        next_params.rotation = angle - 60; // Поворот для создания фрактала
        next_params.depth = params_.depth - 1;
        // Не изменяем толщину линии!
